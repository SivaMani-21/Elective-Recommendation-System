...
def hod_dashboard():
    cur = mysql.connection.cursor()
    try:
        cur.execute("SELECT * FROM elective_selections")
        all_electives = cur.fetchall()

        cur.execute("SELECT elective_name, COUNT(*) FROM elective_selections GROUP BY elective_name")
        elective_counts = cur.fetchall()

        cur.execute("SELECT department, COUNT(*) FROM elective_selections GROUP BY department")
        department_counts = cur.fetchall()

        cur.execute("SELECT year, COUNT(*) FROM elective_selections GROUP BY year")
